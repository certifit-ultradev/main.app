            ...
            {isOpen && (
                <div className={cn('fixed inset-0 bg-[#121313] bg-opacity-100 z-50 flex flex-col p-4 w-full')}>
                    <div className={cn('flex items-center justify-between mb-8')}>
                        <Image src="/logo.png" alt="logo" width={100} height={30} className={cn('w-20')} />
                    ...
                    {options.map((option, i) => {
                        return (
                            <button key={i} type='button' onClick={(e) => { handleClose(); option.handleClick(e); }} className={cn('hover:bg-white hover:text-black text-white font-semibold px-4 py-2 rounded-full mb-4')}>{option.label}</button>
                        )
                    })}
                </div>
            )}
